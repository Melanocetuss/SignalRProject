...
namespace SignalRWebUI.Controllers
{
    public class DiscountController : Controller
    {
        ...
        {
            var client = _clientFactory.CreateClient();
            var responseMessage = await client.PutAsync($"https://localhost:7274/api/Discounts/ChangeStatusToTrue?id={id}", null);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _clientFactory.CreateClient();
            var responseMessage = await client.PutAsync($"https://localhost:7274/api/Discounts/ChangeStatusToFalse?id={id}", null);
